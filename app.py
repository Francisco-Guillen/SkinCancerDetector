...
def index():
    # Main page
    return render_template('index.html') # When you use Flask's render_template() function, it automatically looks for template files in the templates folder
...
def model_predict(img_path, model):
    ...
    prediction = sorted(
      ...
      key=lambda x: x[1] # Specifies that sorting should be based on the second element of each tuple, i.e. the probability of each class.
  )
...
def predict():
    if request.method == 'POST':
        ...
        basepath = os.path.dirname(__file__)# path where the app is
